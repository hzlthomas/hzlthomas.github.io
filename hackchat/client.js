...
var frontpage = [
	...
	"",
	"",
	"这是hack.chat，我汉化的一个版本（也只会这样），鬼知道它支不支持托管在这",
	"你可以自己创建房间，只要在该网址后边加上“?（你想要的地址）”就可以创建一个房间了",
	"房间名称可以随便设定，之后把网址分享给任意一个有已联网的可访问浏览器的电子设备的人，你们就可以开始聊天了",
	"这里没有什么房间列表，不过，我希望你加入这个群聊（即使没什么人）：",
	"",
	
	"?公共房间",
	
	"当然，这里有一个为你准备的随机房间: ?" + Math.random().toString(36).substr(2, 8),
	"",
	
	"如果想要语法突出显示，请这样: ```<language> <the code>``` 其中<language> 是所有已知的编程语言（我也不会用啊）",
	"",
	"这个hack.chat来自github仓库: https://github.com/hack-chat",
	...
	ws.onopen = function () {
		...
		if (!wasConnected) {
			if (location.hash) {
				...
				var newNick = prompt('请设置你的名称:', myNick);
				if (newNick !== null) {
					myNick = newNick;
				} else {
